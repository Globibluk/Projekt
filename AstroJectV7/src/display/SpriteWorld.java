	...
	public void paint(Graphics g)
	{
		...
		for ( int i = 0 ; i < sprites.length; i++ )
			...
			{		
				...
				g2.drawImage(sprites[(i + xOff + dx) % dx][(j + yOff + dy) % dy].getAlternateSprite(),spriteLength*i,spriteLength*j,spriteLength,spriteLength, frame);
		...
		for ( int i = 0; i < world.getHList().size(); i++) {
			...
			g2.drawImage(world.getHList().get(i).getSprite(),spriteLength*x - spriteLength/2,spriteLength*y - spriteLength/2,agentSpriteSize,agentSpriteSize, frame);
		...
		for ( int i = 0; i < world.getCList().size(); i++) {
			...
			g2.drawImage(world.getCList().get(i).getSprite(),spriteLength*x - spriteLength/2,spriteLength*y - spriteLength/2,agentSpriteSize,agentSpriteSize, frame);
